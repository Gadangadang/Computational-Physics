...
for i in range(len(names)):
    ...
    plt.axvspan(np.max(Mon)*0.2, 0.90*np.max(Mon), facecolor='silver', alpha=0.5, label="Middle class (from Toten)") # X coordinate
    plt.axvspan(np.max(Mon)*0.90 , np.max(Mon), facecolor='gold', alpha=0.5, label="Upper class (from Stavanger)") # X coordinate
